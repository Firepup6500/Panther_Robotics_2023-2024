...
class drivetrain extends LinearOpMode {
    ...
    private final int wheelCircumference = 4;
     ...
     */
    private boolean motorAtTarget(DcMotor motor) {
        return !(motor.getTargetPosition()==motor.getCurrentPosition());
    }
    private boolean atTarget(int targetPos, int currentPos) {
    ...
    public void moveDistance(double distance, double speed) {
        ...
        leftFrontMotor.setTargetPosition((int)(rotations * COUNTS_PER_MOTOR_REV));
        leftBackMotor.setTargetPosition((int)(rotations * COUNTS_PER_MOTOR_REV));
        rightFrontMotor.setTargetPosition((int)(rotations * COUNTS_PER_MOTOR_REV));
        rightBackMotor.setTargetPosition((int)(rotations * COUNTS_PER_MOTOR_REV));
        telemetry.addData("ticks ", leftBackMotor.getTargetPosition());
        ...
        while (motorAtTarget(leftFrontMotor) && motorAtTarget(leftBackMotor) && motorAtTarget(rightFrontMotor) && motorAtTarget(rightBackMotor)) {
            ...
            if(distance<0){
                leftFrontMotor.setPower(speed + correction);
                leftBackMotor.setPower(speed + correction);
                rightFrontMotor.setPower(speed - correction);
                rightBackMotor.setPower(speed - correction);
            }else {
                leftFrontMotor.setPower(speed - correction);
                leftBackMotor.setPower(speed - correction);
                rightFrontMotor.setPower(speed + correction);
